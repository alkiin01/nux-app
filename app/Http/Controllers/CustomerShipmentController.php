...
class CustomerShipmentController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('customer_shipment/customer_shipment_index', $data); 
    }
    ...
    public function get_attachment_list(Request $request){     
        ...
        $po_num = $str[0] ;  
        $data['list'] = CustomerShipment::get_attachment_list($po_num);  
        $data['count'] = CustomerShipment::get_attachment_list($po_num)->count();  
        return view('customer_shipment/attachment_list', $data) ;
    }
    ...
    public function get_comment_list(Request $request){     
        ...
        $str = explode("_",Crypt::decryptString(str_replace("-","=", $str_req[0])));   
        $po_num = $str[0] ;  
        $data['list'] = CustomerShipment::get_comment_list($po_num);  
        $data['count'] = CustomerShipment::get_comment_list($po_num)->count();  
        return view('customer_shipment/comment_list', $data) ;
    }

     

    public function get_count_document(Request $request){      
        $data['total_check'] = CustomerShipment::get_count_document_check();  
        $data['total_approve'] = CustomerShipment::get_count_document_approve();  
        $data['total_document'] = CustomerShipment::get_count_document();  
    ...
    public function get_preview_doc(Request $request){  
        ...
        $PackNum = $str[0] ;     
        $data_detail = CustomerShipment::data_detail($PackNum);  
    ...
    public function front_table(Request $request){     
        ...
        $totalData = CustomerShipment::get_transaction_list($search, $status_id)->count();    
        ...
        if(empty($search))
        {            
            $posts = CustomerShipment::get_transaction_list($search, $status_id)
            ->offset($start)
            ->limit($limit)
            ->orderBy($order,$dir)
            ->get();
            ...
            ->orderBy($order,$dir)
            ->get();  
            $totalFiltered = CustomerShipment::get_transaction_list($search, $status_id)->count();
        } 
        $data = array();
        if(!empty($posts))
        { 
    ...
    public function detail_table(Request $request){   
        ...
        $totalData = CustomerShipment::get_detail_transaction_list($search, $PackNum)->count();    
        // dd($totalData);
        $totalFiltered = $totalData;  
        $limit = $request->input('length');
        $start = $request->input('start');
        ...
        if(empty($search))
        {            
            $posts = CustomerShipment::get_detail_transaction_list($search, $PackNum)
            ->offset($start)
            ->limit($limit)
            ->orderBy($order,$dir)
            ->get();
            ...
            ->orderBy($order,$dir)
            ->get();  
            $totalFiltered = CustomerShipment::get_detail_transaction_list($search, $PackNum)->count();
        } 
        $data = array();
        if(!empty($posts))
        { 
    ...
    public function detail_release_table(Request $request){   
        ...
        $totalData = CustomerShipment::get_detail_release_transaction_list($search, $PackNum, $PackLine)->count();    
        // dd($totalData);
        $totalFiltered = $totalData;  
        $limit = $request->input('length');
        $start = $request->input('start');
        ...
        if(empty($search))
        {            
            $posts = CustomerShipment::get_detail_release_transaction_list($search, $PackNum, $PackLine)
            ->offset($start)
            ->limit($limit)
            ->orderBy($order,$dir)
            ->get();
            ...
            ->orderBy($order,$dir)
            ->get();  
            $totalFiltered = CustomerShipment::get_detail_release_transaction_list($search, $PackNum, $PackLine)->count();
        } 
        $data = array();
        if(!empty($posts))
        { 
    ...
    public function export_front_table(Request $request){    
        ...
        $data['list'] = CustomerShipment::get_transaction_list($year, $search, $vendor_id)->get(); 
        $data['num'] = CustomerShipment::get_transaction_list($year, $search, $vendor_id)->count(); 
        $data['ref_form'] = '' ; 
        return view('customer_shipment.po_export', $data); 
    }
    ...
    public function show(Request $request) {         
        ...
        $sys_id = "'".$request->trc_unix_id."'" ;    
        $PONum = $str[0] ;  
        $data_detail = CustomerShipment::data_detail($PONum);    
        foreach ($data_detail as $db) {
    ...
    {  
        ...
        $sysID = $request->sysID ;
        $qty_pack_line = CustomerShipment::get_qty_before($packNum, $packLine) ;
        $qty_pack_line_lot = CustomerShipment::get_qty_lot($sysID) ;
        if ($qty_pack_line <= $qty_pack_line_lot) {
            $data = self::delete_lot($packNum, $packLine);  
        } else {
            $ds_prop = CustomerShipment::get_prop_lot($sysID);
            if ($ds_prop->count() > 0) {
        ...
        if ($data['code'] == 200) {
            $delete_record = CustomerShipment::delete_record_lot($sysID);
        }
        return $data ;
    ...
    { 
        ...
        if ($data['code'] == 200) {
            $clear_record_lot = CustomerShipment::destroy_record_lot($PackNum);
        }
        return $data ;
    }
    ...
    public function set_order_number(Request $request)
    {
        ...
        $data = [];
        $host_api = self::get_host_api() ;
        $check_so = CustomerShipment::check_so_number($orderNum, $packNum)  ;
        if ($check_so > 0) { 
            $client = new Client(); 
            try {
    ...
    {    
        ...
        $PartNum = $int_label[0] ;
        if (substr($int_label[5],0,3) == 'MIT') {
            $LotNum = $int_label[4] ;
        } else {
            $LotNum = $int_label[5] ;
        }
        
        $db_order = CustomerShipment::get_order_line($OrderNum, $PartNum) ;  
        if ($db_order->count() > 1) {
    ...
    {    
        ...
        $db_order = CustomerShipment::get_order_line_by_slip_no($OrderNum, $PartNum, $ext_label) ; 
        if ($db_order->count() == 1) { 
                foreach ($db_order AS $row) {
                    $data['order_line'] = $row->OrderLine ;
                    $data['order_rel'] = $row->OrderRelNum ;
    ...
    { 
        ...
        $pack_line = CustomerShipment::get_pack_line($packNum, $orderNum, $orderLine, $orderRel, $lotNum);
        if ($pack_line > 0) {
          $qty_before = CustomerShipment::get_qty_before($packNum, $pack_line);
          ...
          if ($data['code'] == 200) {
            $insert_record = CustomerShipment::insert_record_pallet_with_pack_line($packNum, $orderNum, $orderLine, $orderRel, $partNum, $lotNum, $lineDesc, $displayInvQty, $poNum, $pack_line, $ext_label);
          }
        } else {
          $data = self::add_detail($packNum, $orderNum, $orderLine, $orderRel, $partNum, $lotNum, $lineDesc, $displayInvQty, $poNum) ;
          if ($data['code'] == 200) {
            $insert_record = CustomerShipment::insert_record_pallet($packNum, $orderNum, $orderLine, $orderRel, $partNum, $lotNum, $lineDesc, $displayInvQty, $poNum, $ext_label);
          }
        }   
        return $data ;
    }
    ...
    public function add_detail($packNum, $orderNum, $orderLine, $orderRel, $partNum, $lotNum, $lineDesc, $displayInvQty, $poNum)
    {
        $username = Auth::user()->username ;
        $password = Crypt::decryptString(Auth::user()->epicor_password) ;  
        $lineDesc = str_replace("__", ",", $lineDesc) ;  

...
<script>
    $(document).ready(function() {
        ...
        var groupOption = new Option(
            "{{ $data->group_name }}", // text — sesuaikan nama kolom
            "{{ $data->group_id }}", // value
            true, true
        );
        $('#group_id').append(groupOption).trigger('change');
        var subGroupOption = new Option(
            "{{ $data->sub_group_name }}", // text — sesuaikan nama kolom
            "{{ $data->sub_group_id }}", // value
            true, true
        );
        $('#sub_group_id').append(subGroupOption).trigger('change');
    })
    ...
    $("#btn_save").on('click', function() {
        ...
        $.ajax({
            url: "{{ route('data_menu.save_menu_edit') }}",
            type: 'post',
            data: {
                _token: '{{ csrf_token() }}',
                ref_doc: ref_doc,
